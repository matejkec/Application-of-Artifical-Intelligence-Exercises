...
def FintessFunction(population): 
    ...
    for i in range(len(population)): 
        result = 0 
        # jednadzba: 2*k + 3*u + 10*d - 9*w - 5*y + 10*k + 4*q - 2*y - 7*s = 36
        # indexi: k:0, u:1, d:2, w:3, y:4, q:5, s:6
        result = abs((2*population[i][0] + 3*population[i][1] + 10*population[i][2]
                      - 9*population[i][3] - 5*population[i][4] + 10*population[i][0]
                      + 4*population[i][5] - 2*population[i][4] - 7*population[i][6]) 
                     - 36)
        F_obj.append(result)
